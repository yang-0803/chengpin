...
# 云端/本地统一下载字体到临时目录，无需手动上传
font_path = "/tmp/NotoSansSC-Regular.ttf"
if not os.path.exists(font_path):
    try:
        # 国内可访问字体直链
        font_url = "https://cdn.jsdelivr.net/gh/googlefonts/notosanssc@static/fonts/NotoSansSC-Regular.ttf"
        urllib.request.urlretrieve(font_url, font_path)
    except Exception as err:
        st.sidebar.warning("字体下载失败，图表将使用英文字体")
...
if os.path.exists(font_path):
    ...
    st.sidebar.success(f"✅ 绘图中文字体加载完成：{font_name}")
else:
    # 兜底纯英文字体
    plt.rcParams["font.sans-serif"] = ["DejaVu Sans"]
    plt.rcParams["axes.unicode_minus"] = False

# 页面配置【只保留一次，删除重复的set_page_config】
st.set_page_config(
...
def load_and_preprocess_data():
    ...
    for old_name, new_name in area_mapping.items():
        valid_data.rename(columns={old_name}, inplace=True)
...
# ===================== 二、区域用电分析 =====================
    ...
    for idx, area in enumerate(pivot.columns):
        ax.plot(month_labels, pivot[area], marker='o', linewidth=2, label=area, color=all_colors)
